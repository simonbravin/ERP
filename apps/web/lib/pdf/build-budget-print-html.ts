...
export function buildBudgetPrintHtml(
  ...
  const hasLegal = [legalLine, layout.address, layout.email, layout.phone].some(
    (v) => v != null && String(v).trim() !== ''
  )
  const projectLine =
    layout.orgName && page.projectName
      ? (page.projectNumber ? `${page.projectNumber} — ` : '') + page.projectName
      : null
  const folioLine = `Versión: ${esc(page.versionCode)}`
  const issuedLine =
    layout.userNameOrEmail != null && layout.userNameOrEmail.trim() !== ''
      ? `Emitido por: ${esc(layout.userNameOrEmail.trim())}`
      : null

  const headerHtml = `
<header style="padding-bottom:0.75rem;margin-bottom:1rem;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;">
  <div style="display:flex;align-items:center;gap:0.75rem;min-width:0;max-width:65%;">
    ${layout.logoUrl ? `<img src="${esc(layout.logoUrl)}" alt="" style="height:2.5rem;width:auto;object-fit:contain;" />` : ''}
    <div>
  ...
  if (info?.endDate != null && String(info.endDate).trim() !== '') projectInfoRows.push(`<div><strong>Fin:</strong> ${esc(String(info.endDate))}</div>`)
  ...
  if (info?.description != null && String(info.description).trim() !== '') projectInfoRows.push(`<div><strong>Detalles:</strong> ${esc(String(info.description))}</div>`)
  ...
  const rowsHtml = sorted.map((r) => {
    ...
    return `<tr>
          <td style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;padding-left:${paddingLeft + 8}px;">${esc(r.code)}</td>
          <td style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;padding-left:${paddingLeft + 8}px;">${esc(r.description)}</td>
          <td style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:center;">${esc(r.unit)}</td>
          <td style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:right;">${r.quantity}</td>
          <td style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:right;">${esc(formatCurrency(r.unitPrice))}</td>
          <td style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:right;">${esc(formatCurrency(r.totalCost))}</td>
        </tr>`
  })

  const title =
    page.projectNumber && page.projectNumber.trim() !== ''
      ? `Presupuesto — ${esc(page.projectName)} (${esc(page.projectNumber)}) — ${esc(page.versionCode)}`
      : `Presupuesto — ${esc(page.projectName)} — ${esc(page.versionCode)}`

  const tableHtml = `
<div class="document-title" style="margin-top:24px;margin-bottom:0.5rem;">
...
<table style="width:100%;border-collapse:collapse;font-size:0.8125rem;">
  <thead>
    <tr>
      <th style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:left;background:#f3f4f6;font-weight:600;">Código</th>
      <th style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:left;background:#f3f4f6;font-weight:600;">Descripción</th>
      <th style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:center;background:#f3f4f6;font-weight:600;">Und</th>
      <th style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:right;background:#f3f4f6;font-weight:600;">Cantidad</th>
      <th style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:right;background:#f3f4f6;font-weight:600;">P.Unit</th>
      <th style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;text-align:right;background:#f3f4f6;font-weight:600;">Total</th>
    </tr>
  </thead>
  <tbody>${rowsHtml.join('')}</tbody>
  <tfoot>
    <tr>
      <td colspan="5" style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;font-weight:700;background:#f3f4f6;">Total presupuesto</td>
      <td style="padding:0.375rem 0.5rem;border:1px solid #e2e8f0;font-weight:700;background:#f3f4f6;text-align:right;">${esc(formatCurrency(page.grandTotal))}</td>
    </tr>
  </tfoot>
</table>`
